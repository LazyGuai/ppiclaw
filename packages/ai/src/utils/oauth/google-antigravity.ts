...
const CLIENT_ID = process.env.PI_GOOGLE_ANTIGRAVITY_OAUTH_CLIENT_ID;
const CLIENT_SECRET = process.env.PI_GOOGLE_ANTIGRAVITY_OAUTH_CLIENT_SECRET;
...
function assertOAuthClientConfigured(): { clientId: string; clientSecret: string } {
	if (!CLIENT_ID || !CLIENT_SECRET) {
		throw new Error(
			[
				"Google Antigravity OAuth client is not configured.",
				"Set PI_GOOGLE_ANTIGRAVITY_OAUTH_CLIENT_ID and PI_GOOGLE_ANTIGRAVITY_OAUTH_CLIENT_SECRET in your environment.",
			].join(" "),
		);
	}
...
export async function refreshAntigravityToken(refreshToken: string, projectId: string): Promise<OAuthCredentials> {
	const { clientId, clientSecret } = assertOAuthClientConfigured();
